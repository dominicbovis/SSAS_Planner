...
import { SsasScheme, ThirdPartyLoanRecord } from '../types';
import Modal from '../components/Modal';

const empty = (): Omit<ThirdPartyLoanRecord, 'id' | 'scheme_id' | 'created_at'> => ({
  borrower_name: '', loan_amount: 0, interest_rate: 0, loan_date: null,
...
function ThirdPartyLoanRegister({ scheme }: Props) {
  const [records, setRecords] = useState<ThirdPartyLoanRecord[]>([]);
  const [modal, setModal] = useState<{ mode: 'add' | 'edit'; record: Omit<ThirdPartyLoanRecord, 'id' | 'scheme_id' | 'created_at'>; id?: string } | null>(null);
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('third_party_loans').select('*').eq('scheme_id', scheme.id).order('created_at');
    setRecords(data ?? []);
    setLoading(false);
  }
  ...
  async function save() {
    if (!modal) return;
    setSaving(true);
    if (modal.mode === 'add') await supabase.from('third_party_loans').insert({ ...modal.record, scheme_id: scheme.id });
    else await supabase.from('third_party_loans').update(modal.record).eq('id', modal.id!);
    setSaving(false);
    setModal(null);
    load();
  }
  ...
  async function remove(id: string) {
    ...
    await supabase.from('third_party_loans').delete().eq('id', id);
    load();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Third-Party Loan Register</h1>
          <p className="text-sm text-gray-500 mt-0.5">Loans to unconnected third parties — Total outstanding: <strong>{fmt(total)}</strong></p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-400 text-sm">No third-party loans recorded.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  {['Borrower', 'Loan Amount', 'Interest Rate', 'Loan Date', 'Repayment Date', 'Outstanding Balance', 'Security', ''].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {records.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3 font-medium text-gray-800">{r.borrower_name}</td>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-1">
                        <button onClick={() => setModal({ mode: 'edit', record: { borrower_name: r.borrower_name, loan_amount: Number(r.loan_amount), interest_rate: Number(r.interest_rate), loan_date: r.loan_date, repayment_date: r.repayment_date, outstanding_balance: Number(r.outstanding_balance), security: r.security, notes: r.notes }, id: r.id })} className="p-1.5 rounded text-gray-400 hover:text-blue-600 hover:bg-blue-50"><Pencil size={14} /></button>
      ...
      {modal && (
        <Modal title={modal.mode === 'add' ? 'Add Loan' : 'Edit Loan'} onClose={() => setModal(null)}>
          <div className="grid grid-cols-2 gap-4">
            {[
              { label: 'Borrower Name', field: 'borrower_name', type: 'text', span: 2 },
...
export default ThirdPartyLoanRegister